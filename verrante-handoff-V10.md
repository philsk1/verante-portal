# VERRANTE — COMPLETE PROJECT HANDOFF DOCUMENT V10
...
(both call_outcome and triage_outcome stored — same value — for compatibility with both portal tabs)

**demo_users:**
id, email, name, role, access_code
...
### Done — all portal tabs + infrastructure
...
- All demo_ tables created in Supabase
- 10 businesses seeded with full data (call logs, leads, referrals, partners, staff, services)
- ~480 call logs with today's calls + 26-day history, outcomes tuned per business
- 50 leads with realistic mix of statuses (2-3 actionable new per business)
- 40 referral log entries
- Pricing + competitor intelligence for Restore Physiotherapy (Enterprise demo)
- demo@verrante.app / VERRANTE2026 in demo_users
...
### Demo Session 3 (after Session 2)
...
DemoContext (Session 2) will query demo_ tables via Supabase anon client. No Supabase auth is used for demo — JWT stored in localStorage after email check against demo_users.

DemoContext will need to shape data to match what portal tabs expect:
- ActivityDashboard expects: call_outcome, duration_seconds, ai_summary, caller_phone/callers join
  → In demo: caller_name serves as caller display, call_outcome and duration_seconds are direct columns
- DataAnalytics expects: triage_outcome, duration
  → In demo: triage_outcome stored directly; map duration_seconds → duration in DemoContext
- referral_log expects: referral_partners(business_name)
  → In demo: partner_name stored directly in demo_referral_log
- leads expects: lead_contact_name, callers(phone_number)
  → In demo: caller_name and caller_number stored directly
