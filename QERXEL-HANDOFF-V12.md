...
Demo login: demo@qerxel.app / QERXEL2026
...
2. Stripe Dashboard → Webhooks → Add endpoint: `https://qerxel-portal.vercel.app/api/stripe-webhook` → events: checkout.session.completed, customer.subscription.updated, customer.subscription.deleted → copy whsec_ secret.
...
- **Qerxel Calendar Sessions 3–5** (see Strategy Addendum V2 for full spec — Sessions 1 and 2 complete)
